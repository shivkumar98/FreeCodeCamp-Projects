...
## 🎬 Demo 🎬
* You can test out my application which I built for this project [here](https://htmlpreview.github.io/?https://github.com/shivkumar98/FreeCodeCamp-Projects/blob/main/05-javascript-a-ds-new/0-certification-projects/3-build-telephone-number-validator/code/index.html)
...
* Here is my whole function so far:
   ```js
   function isPhoneNumberValid(phoneNumber) {
      const phoneNumberSplit = phoneNumber.split("")
      if (phoneNumberSplit[0] == "-") return false
   ...
   ```
* I clear up some duplication with regexes, and clean up formatting
* Here is the final implementation of the `isPhoneNumberValid()` function:
   ```js
   function isPhoneNumberValid(phoneNumber) {
      const phoneNumberSplit = phoneNumber.split("")
      if (phoneNumberSplit[0] == "-") return false
